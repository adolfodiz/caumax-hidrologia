# dem25_tab.py (Versión con Corrección de AttributeError y Lógica de UI)
...
# ==============================================================================
# SECCIÓN 3: LÓGICA DE ANÁLISIS HIDROLÓGICO (CORREGIDA)
# ==============================================================================
...
def delinear_cuenca_desde_punto(_dem_bytes, outlet_coords_wgs84, umbral_rio_export):
    ...
    try:
        ...
        results.update({
            ...
            "pysheds_data": {
                # --- ¡¡¡CORRECCIÓN CLAVE 1!!! ---
                # Guardamos el objeto 'grid' completo, no solo sus resultados.
                "grid": grid,
                # --- FIN CORRECCIÓN ---
...
def calcular_morfometria_cuenca(_pysheds_data, umbral_rio_export):
    ...
    try:
        # --- ¡¡¡CORRECCIÓN CLAVE 2!!! ---
        # Recuperamos el objeto 'grid' que guardamos en el paso anterior.
        ...
        # --- ¡¡¡CORRECCIÓN CLAVE 3!!! ---
        # Llamamos al método desde la instancia 'grid', no desde la clase 'Grid'.
...
# ==============================================================================
# SECCIÓN 5: FUNCIÓN PRINCIPAL DEL FRONTEND (RENDERIZADO CORREGIDO)
# ==============================================================================
...
def render_dem25_tab():
    ...
    with b_col3:
        if st.button("3. Generar Gráficos", use_container_width=True, disabled=not st.session_state.get('morphometry_data')):
            results = generar_graficos_y_analisis(st.session_state.pysheds_data, st.session_state.morphometry_data)
            if results['success']:
                st.session_state.visualization_data = results['visualization_data']
                st.success("Paso 3 completado."); st.rerun()
            else: st.error("Falló el Paso 3."); st.code(results['message'])

    # --- ¡¡¡CORRECCIÓN CLAVE 4!!! ---
    # Lógica de visualización de resultados por pasos
    if st.session_state.get('pysheds_data'):
        ...
        with res1_col1:
            ...
            folium.GeoJson(gdf_cuenca, name="Cuenca Delineada", style_function=lambda x: {'color': '#FF0000', 'weight': 2.5, 'fillOpacity': 0.2}).add_to(m_results_1)
            m_results_1.fit_bounds(gdf_cuenca.total_bounds[[1, 0, 3, 2]].tolist())
        ...
        with res1_col2:
            ...
            st.download_button("📥 Descargar Cuenca (.zip)", export_gdf_to_zip(gdf_cuenca, "cuenca_delineada"), "cuenca_delineada.zip", use_container_width=True)
        ...
        if st.session_state.get('morphometry_data'):
            ...
            with res2_col1:
                ...
                folium.GeoJson(gdf_cuenca, name="Cuenca", style_function=lambda x: {'color': '#FF0000', 'weight': 1, 'fillOpacity': 0.1}).add_to(m_results_2)
                folium.GeoJson(gdf_lfp, name="LFP", style_function=lambda x: {'color': '#FFFF00', 'weight': 4}).add_to(m_results_2)
                if not gdf_rios_recortado.empty:
                    folium.GeoJson(gdf_rios_recortado, name="Red Fluvial", style_function=lambda f: {'color': 'blue', 'weight': f['properties']['strord'] / 2 + 1}).add_to(m_results_2)
                m_results_2.fit_bounds(gdf_cuenca.total_bounds[[1, 0, 3, 2]].tolist()); folium.LayerControl().add_to(m_results_2)
            ...
            with res2_col2:
                ...
                st.metric("Longitud", f"{metrics.get('longitud_m', 0):.2f} m")
                st.metric("Pendiente", f"{metrics.get('pendiente_media', 0):.4f} m/m")
                st.metric("T. Concentración", f"{metrics.get('tc_h', 0):.3f} h")
                st.download_button("📥 Descargar LFP (.zip)", export_gdf_to_zip(gdf_lfp, "lfp"), "lfp.zip", use_container_width=True)
                st.download_button("📥 Descargar Red Fluvial (.zip)", export_gdf_to_zip(gdf_rios, "red_fluvial"), "red_fluvial.zip", use_container_width=True)
        ...
        if st.session_state.get('visualization_data'):
            ...
            st.image(io.BytesIO(base64.b64decode(plots['grafico_1_mosaico'])), caption="Características de la Cuenca", use_container_width=True)
